...
export class CakedetailsComponent implements OnInit {
  ...
  constructor(private route:ActivatedRoute , private arunservice:ArunService , private router:Router) {
    this.cakeid=this.route.snapshot.params["cakeid"]
    var url="https://apifromashu.herokuapp.com/api/cake/" + this.cakeid
    
    this.arunservice.showCakeDetails(url).subscribe({
      next:(response:any)=>{
   ...
   addtocart(){
     if(localStorage["token"]){
      ...
      this.arunservice.addCakeToCart(url,body,options).subscribe({
        next:(response:any)=>{
